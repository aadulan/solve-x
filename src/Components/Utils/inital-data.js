...
export const equationGen = (name) => {
  ...
  if (name === 'level1') {
    if (Math.random() >= 0.5) {
      return `${randomNumber + splitTerm} * x +${splitConstant} = ${splitTerm} * x + ${
        constant + splitConstant
      }`;
    } else {
    ...
    if (Math.random() >= 0.5) {
      return `${
        randomNumber + splitTerm
      } * x /${randomFrac} +${splitConstant} = ${splitTerm} * x + ${
        constant + splitConstant
      }`;
    } else {
      return `${randomNumber} * x/${randomFrac} +${splitConstant} = ${
        constant + splitConstant
      }`;
    }
  } else if (name === 'level4') {
    if (Math.random() >= 0.5) {
      return `-${
        randomNumber + splitTerm
      } * x/${randomFrac} = -${splitTerm} * x + ${constant}`;
    } else {
      return `${-randomNumber} * x/${randomFrac} -${splitConstant} = ${
        constant - splitConstant
      }`;
