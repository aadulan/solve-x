...
export const displayExpression = (
  expression,
  side,
  isShowSign,
  isUnpack,
  ...
  expression.terms.forEach((t, index) => {
    ...
    var unpack = isUnpack
      ? t.coefficients[0].numer + '\\times ' + t.variables[0].variable
      : coeff + t.variables[0].variable;
  ...
  if (expression.terms.length === 0) {
    ...
    factors = new Set([...factor_exp].filter((x) => factor_const.has(x)));
  }
